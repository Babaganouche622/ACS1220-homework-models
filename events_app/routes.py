...
def create():
    ...
    if request.method == 'POST':
        ...
        create_new_event = Event(title=new_event_title, description=new_event_description, date_and_time=date_and_time)
...
def rsvp(event_id):
    ...
    if is_returning_guest:
        ...
        if guest is None:
            error = "ERROR Guest was not found!!"
            flash(error)
        else:
            guest.events_attending.append(this_event)
            db.session.commit()
            flash("See you at the party!!!")
